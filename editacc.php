...
<fieldset>
    ...
    <label>Change Mark </Title>: </label><br>
    
    <input type="number" name="mark" placeholder="Enter New Mark"><br><br>
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
       ...
       if($mark==''){
        ...
        echo "A number is entered";
       }

       if($check==2){
        if($subject=='Physical Properties of Materials 1'){
            $sql2="UPDATE screcord SET scQuiz1='$mark' WHERE Name='$name'";
            mysqli_query($conn, $sql2);
        }
        if($subject=='All About Bacteria 1'){
            $sql2="UPDATE screcord SET scQuiz2='$mark' WHERE Name='$name'";
            mysqli_query($conn, $sql2);
        }
        if($subject=='Addition and Subtraction for Kids'){
            $sql2="UPDATE mathrecord SET mathQuiz1='$mark' WHERE Name='$name'";
            mysqli_query($conn, $sql2);
        }
        if($subject=='Multiplication Table'){
            $sql2="UPDATE mathrecord SET mathQuiz2='$mark' WHERE Name='$name'";
            mysqli_query($conn, $sql2);
        }
        if($subject=='What is an Engineer'){
            $sql2="UPDATE engrecord SET engQuiz1='$mark' WHERE Name='$name'";
            mysqli_query($conn, $sql2);
        }
        if($subject=='Jessi Has a Problem'){
            $sql2="UPDATE engrecord SET engQuiz2='$mark' WHERE Name='$name'";
            mysqli_query($conn, $sql2);
        }
        if($subject=='Technology for Kids 1'){
            $sql2="UPDATE techrecord SET techQuiz1='$mark' WHERE Name='$name'";
            mysqli_query($conn, $sql2);
        }
        if($subject=='Technology for Kids 2'){
            $sql2="UPDATE techrecord SET techQuiz2='$mark' WHERE Name='$name'";
            mysqli_query($conn, $sql2);
        }
